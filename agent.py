...
PROFILES = {1: {1080: 50}, 2: {1080: 30}, 3: {1080: 20}, 4: {1080: 15}, 5: {1080: 10}, 6: {1080: 5}, 7: {720: 25}, 8: {720: 15}, 9: {720: 10}, 10: {720: 7.5}, 11: {720: 5}, 12: {720: 2.5}}
    ...
    with tf.Session() as sess, open(LOGS_DIR + '_test', 'w') as log_test_file:
        ...
        while True:
            ...
            for i in range(NUM_AGENTS):
                net_params_queues[i].put([actor_params, critic_params])#Do not forget to check asynchronous way
            ...
            for i in range(NUM_AGENTS):
                states_matrix, actions_matrix, rewards_matrix, terminal,
                    info = exp_queues[i].get()

                actor_gradients, critic_gradients, td_matrix = \
            ...
            epoch += 1
            td_loss_avg = total_td_loss / float(NUM_AGENTS)
            reward_avg = total_reward / rew_batch_len
            entropy_avg = total_entropy / ent_batch_len

            logging.info('Epoch: ' + str(epoch) + ' TD Loss: ' + str(td_loss_avg) +
                         ' Average Reward: ' + str(reward_avg))
...
def agent(agent_id, traces, net_params_queue, exp_queue):  # General agent
    ...
    with tf.Session() as sess, open(LOGS_DIR + 'agent' + str(agent_id), 'w') as log_file:#Open another file to easily plot
        ...
        while True:  #Maybe handle it until precision model reaches some error point
            ...
            #Possible pseudo MOS
            birate_in = list(PROFILES[action].values())[0]  # Bitrate tx
            birate_out = list(PROFILES[profile].values())[0]  # Bitrate rx
            ...
            curr_state[0, -1] = video_bitrates[bitrate_rec]  #Latest bitrate
            curr_state[1, -1] = float(mean_free_capacity_frac)  #Free capacity of the link
            cuur_state[2, -1] = float(mean_loss_rate_frac)  #Check frac
            ...
            #entropy_matrix.append(actor_net.compute_entropy(action_predicted[0]))#Add it
...
def main():
    ...
    #Video information. First take local video, later adapt to streaming
    #video_bitrates = [3, 5, 10, 15, 20, 50]

    # Background traffic from iperf
    random_traffic
